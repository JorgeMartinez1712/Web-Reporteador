        ...
        <>
          <div>
            ...
            {selectedInventoryUnit ? (
              <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200 flex flex-col space-y-2 text-left mt-4">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-semibold text-emerald-800 text-lg">{selectedInventoryUnit.product.name}</p>
                    <p className="text-emerald-700 text-sm">SKU: {selectedInventoryUnit.product.sku || 'N/A'}</p>
                    <p className="text-emerald-700 text-sm">Serial: {selectedInventoryUnit.serial_number || 'N/A'}</p>
                    <p className="text-emerald-700 text-sm">IMEI: {selectedInventoryUnit.imei || 'N/A'}</p>
                    <p className="text-emerald-700 text-sm">Sucursal: {selectedInventoryUnit.retail_unit?.name || 'N/A'}</p>
                    <p className="text-emerald-700 text-sm">Marca: {selectedInventoryUnit.product.brand || 'N/A'}</p>
                    <p className="text-emerald-700 text-sm">Categoría: {selectedInventoryUnit.product.category || 'N/A'}</p>
                  </div>
                  {!isProductSaved && (
                    <button
                      type="button"
                ...
                <div className="mt-2 pt-2 border-t border-emerald-200 text-right">
                  <p className="text-lg font-bold text-emerald-900">Precio Unitario: {selectedInventoryUnit?.product?.currency?.symbol}{parseFloat(selectedInventoryUnit?.product?.price).toFixed(2)}</p>
                </div>
              </div>
            ) : (
              <>
                      ...
                      {allInventoryUnits.map((unit) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <span className="font-bold text-emerald-600">{unit.product.currency?.symbol}{parseFloat(unit.product.price).toFixed(2)}</span>
          ...
          <div className="flex justify-between mt-6">
            ...
            <button
              type="button"
              onClick={handleNext}
              className={`bg-emerald-600 text-white py-2 px-4 rounded-lg shadow transition duration-200 font-medium ${!selectedInventoryUnit ? 'opacity-50 cursor-not-allowed' : 'hover:bg-emerald-700'}`}
