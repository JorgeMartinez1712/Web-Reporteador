...
const SidebarSection = ({ title, items, isCollapsed, icon, titleIcon }) => {
    ...
    const titleIconColor = 'text-emerald-500';
    const activeSectionColor = 'text-emerald-500';
    ...
    return (
        <div className="mt-4">
            <div
                className={`group flex items-center cursor-pointer p-2 rounded-md hover:bg-gray-200 transition-colors duration-200 ${
                    isCollapsed ? 'justify-center' : 'justify-between'
                }`}
                onClick={() => setIsSectionCollapsed(!isSectionCollapsed)}
            >
                <i className={sectionIconClass}></i>
                {!isCollapsed && <h3 className="text-base font-medium text-gray-700 flex-grow">{title}</h3>}
                {!isCollapsed && <i
                    className={`bi text-gray-600 text-sm transition-transform duration-200 ${
