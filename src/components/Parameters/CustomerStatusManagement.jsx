...
const CustomerStatusManagement = () => {
  const { data: customerStatuses, loading, error, fetchData, createItem, updateItem, updateItem2, deleteItem } = useParameters('/custo-status');
  ...
  const handleCreateOrUpdate = async (e) => {
    e.preventDefault();
    if (!statusName.trim()) {
      alert('Por favor, completa el nombre del estado de cliente.');
      return;
    }

    try {
      if (isEditing) {
        await updateItem2('/custo-status/update', { id: currentStatus.id, code: statusCode, name: statusName, description: statusDescription });
      } else {
      ...
      console.error('Error al guardar estado de cliente:', err);
      alert(`Error al guardar estado de cliente: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este estado de cliente?')) {
      try {
        await deleteItem(id);
        fetchData();
      } catch (err) {
        console.error('Error al eliminar estado de cliente:', err);
        alert(`Error al eliminar estado de cliente: ${err.response?.data?.message || err.message}`);
      }
    }
  };
    ...
    <div className="p-4">
      <form onSubmit={handleCreateOrUpdate} className="mb-6 p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label htmlFor="statusCode" className="block text-gray-700 text-sm font-bold mb-2">Código:</label>
            <input
              type="text"
              id="statusCode"
          ...
          <div>
            <label htmlFor="statusName" className="block text-gray-700 text-sm font-bold mb-2">Nombre:</label>
            <input
              type="text"
              id="statusName"
              className="border border-gray-300 rounded-lg p-2 w-full text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-fuchsia-500"
        ...
        <div className="mb-4">
          <label htmlFor="statusDescription" className="block text-gray-700 text-sm font-bold mb-2">Descripción:</label>
          <textarea
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {customerStatuses && customerStatuses.length === 0 ? (
                <tr>
                  <td colSpan="4" className="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                    No hay estados de cliente.
                  </td>
                </tr>
              ) : (
                customerStatuses && customerStatuses.map((status, index) => (
                  <tr key={status.id}>
