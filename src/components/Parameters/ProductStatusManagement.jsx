...
const ProductStatusManagement = () => {
  const { data: statuses, loading, error, fetchData, createItem, updateItem2, deleteItem } = useParameters('/product-statuses', 'data');
  ...
  const handleCreateOrUpdate = async (e) => {
    ...
    if (!statusCode.trim() || !statusName.trim()) {
      alert('Por favor, completa el Código y el Nombre del estado del producto.');
      return;
    }

    try {
      if (isEditing) {
        await updateItem2('/product-statuses/update', { id: currentStatus.id, code: statusCode, name: statusName, description: statusDescription });
      } else {
      ...
      console.error('Error al guardar estado de producto:', err);
      alert(`Error al guardar estado de producto: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este estado de producto?')) {
      try {
        await deleteItem(id);
        fetchData();
      } catch (err) {
        console.error('Error al eliminar estado de producto:', err);
        alert(`Error al eliminar estado de producto: ${err.response?.data?.message || err.message}`);
      }
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              ...
              disabled={isEditing}
              placeholder="Ej: ACTIVE, INACTIVE"
              required
            />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              placeholder="Una breve descripción del estado del producto"
            ></textarea>
          </div>
        </div>
...
export default ProductStatusManagement;