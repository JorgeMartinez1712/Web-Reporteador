...
const DeviceLockStatusManagement = () => {
    const { data, loading, error, fetchData, createItem, updateItem, deleteItem } = useParameters('/device-lock-statuses');
    const [deviceLockStatuses, setDeviceLockStatuses] = useState([]);
    ...
    useEffect(() => {
        if (data && Array.isArray(data.data)) {
            setDeviceLockStatuses(data.data);
        } else {
            setDeviceLockStatuses([]);
        }
    }, [data]);
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        if (!statusName.trim() || !statusCode.trim()) {
            alert('Por favor, ingresa el Nombre y el Código del estado de bloqueo de dispositivo.');
            return;
        }
        ...
        try {
            ...
            console.error('Error al guardar estado de bloqueo de dispositivo:', err);
            alert(`Error al guardar estado de bloqueo de dispositivo: ${err.response?.data?.message || err.message}`);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de que quieres eliminar este estado de bloqueo de dispositivo?')) {
            try {
                await deleteItem(id);
                fetchData();
            } catch (err) {
                console.error('Error al eliminar estado de bloqueo de dispositivo:', err);
                alert(`Error al eliminar estado de bloqueo de dispositivo: ${err.response?.data?.message || err.message}`);
            }
        }
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input
                            ...
                            disabled={isEditing}
                            placeholder="Ej: LOCKED, UNLOCKED, PENDING (MAYÚSCULAS y sin espacios)"
                            required
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej: Bloqueado, Desbloqueado, Pendiente"
                            required
                        />
                    </div>
                    <div className="md:col-span-2">
                        ...
                        <textarea
                            ...
                            placeholder="Una breve descripción del estado de bloqueo del dispositivo"
                        ></textarea>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {Array.isArray(deviceLockStatuses) && deviceLockStatuses.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                                        No hay estados de bloqueo de dispositivo.
                                    </td>
                                </tr>
                            ) : (
                                Array.isArray(deviceLockStatuses) && deviceLockStatuses.map((status, index) => (
                                    <tr key={status.id}>
...
export default DeviceLockStatusManagement;