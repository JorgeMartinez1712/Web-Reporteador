...
const PaymentMethodStatusManagement = () => {
    const { data, loading, error, fetchData, createItem, updateItem, deleteItem } = useParameters('/payment-method-statuses');
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        if (!statusName.trim() || !statusCode.trim()) {
            alert('Por favor, ingresa el Nombre y el Código del estado del método de pago.');
            return;
        }
        ...
        try {
            ...
            console.error('Error al guardar estado del método de pago:', err);
            alert(`Error al guardar estado del método de pago: ${err.response?.data?.message || err.message}`);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de que quieres eliminar este estado del método de pago?')) {
            try {
                await deleteItem(id);
                fetchData();
            } catch (err) {
                console.error('Error al eliminar estado del método de pago:', err);
                alert(`Error al eliminar estado del método de pago: ${err.response?.data?.message || err.message}`);
            }
        }
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input
                            ...
                            disabled={isEditing}
                            placeholder="Ej: ACTIVE, INACTIVE, RETIRED (MAYÚSCULAS y sin espacios)"
                            required
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej: Activo, Inactivo, Retirado"
                            required
                        />
                    </div>
                    <div className="md:col-span-2">
                        ...
                        <textarea
                            ...
                            placeholder="Una breve descripción del estado del método de pago"
                        ></textarea>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {methodStatuses && methodStatuses.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                                        No hay estados de método de pago.
                                    </td>
                                </tr>
                            ) : (
                                methodStatuses && methodStatuses.map((status, index) => (
                                    <tr key={status.id}>
...
export default PaymentMethodStatusManagement;