...
const SaleStatusManagement = () => {
    const { data: statuses, loading, error, fetchData, createItem, updateItem, deleteItem } = useParameters('/sale-statuses');
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        if (!statusCode.trim() || !statusName.trim()) {
            alert('Por favor, completa el Código y el Nombre del estado de venta.');
            return;
        }
        ...
        try {
            ...
            console.error('Error al guardar estado de venta:', err);
            alert(`Error al guardar estado de venta: ${err.response?.data?.message || err.message}`);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de que quieres eliminar este estado de venta?')) {
            try {
                await deleteItem(id);
                fetchData(); 
            } catch (err) {
                console.error('Error al eliminar estado de venta:', err);
                alert(`Error al eliminar estado de venta: ${err.response?.data?.message || err.message}`);
            }
        }
    };
...
export default SaleStatusManagement;