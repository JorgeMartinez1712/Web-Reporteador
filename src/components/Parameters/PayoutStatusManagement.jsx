...
const PayoutStatusManagement = () => {
    const { data, loading, error, fetchData, createItem, updateItem, deleteItem } = useParameters('/payout-statuses', 'data');
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        if (!statusName.trim() || !statusCode.trim()) {
            alert('Por favor, ingresa el Nombre y el Código del estado de desembolso.');
            return;
        }
        ...
        try {
            ...
            console.error('Error al guardar estado de desembolso:', err);
            alert(`Error al guardar estado de desembolso: ${err.response?.data?.message || err.message}`);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de que quieres eliminar este estado de desembolso?')) {
            try {
                await deleteItem(id);
                fetchData();
            } catch (err) {
                console.error('Error al eliminar estado de desembolso:', err);
                alert(`Error al eliminar estado de desembolso: ${err.response?.data?.message || err.message}`);
            }
        }
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input
                            ...
                            disabled={isEditing}
                            placeholder="Ej: PENDING, APPROVED, REJECTED (MAYÚSCULAS y sin espacios)"
                            required
                        />
                    </div>
                    <div>
                    ...
                    <div className="md:col-span-2">
                        ...
                        <textarea
                            ...
                            placeholder="Una breve descripción del estado de desembolso"
                        ></textarea>
                    </div>
                </div>
            ...
            <div className="border border-gray-300 rounded-lg overflow-hidden shadow-sm overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {payoutStatuses && payoutStatuses.length === 0 ? (
                            <tr>
                                <td colSpan="5" className="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                                    No hay estados de desembolso.
                                </td>
                            </tr>
                        ) : (
                            payoutStatuses && payoutStatuses.map((status, index) => (
                                <tr key={status.id}>
