    ...
    return (
      <div className="space-y-4 text-left">
        <p className="font-semibold text-gray-700">
          Cliente: <span className="text-oscuro">{customer.full_name}</span> (C.I: {customer.document_number})
        </p>
  ...
  return (
    <div className="space-y-6 text-left">
      <h3 className="text-2xl font-semibold text-gray-800 border-b pb-4 mb-4 border-gray-200">
        Cliente: <span className="text-oscuro">{customer.full_name}</span>
      </h3>
        ...
        {sales.map((sale) => {
          ...
          return (
            <div
              ...
              className={`p-4 border-2 rounded-lg shadow-sm hover:shadow-md transition duration-150 flex justify-between items-center cursor-pointer ${isSelected ? 'border-claro bg-bg shadow-lg' : 'border-gray-200 bg-white hover:border-claro'}`}
            >
              ...
              {isSelected && (
                <FaCheckCircle className="h-6 w-6 text-claro" />
              )}
            </div>
          );
        })}
