...
const InventoryImportModal = ({
  ...
  const handleDownloadFormat = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'SUCURSAL', key: 'SUCURSAL', width: 8 },
      { header: 'IMEI', key: 'IMEI', width: 18, style: { numFmt: '0' } },
      { header: 'NUMERO DE SERIAL', key: 'NUMERO DE SERIAL', width: 18, style: { numFmt: '0' } },
    ];

    const data = [
      {SKU: 11112222, SUCURSAL: 'BIT MERCADO CDA LA CASTELLANA, C.A.',   IMEI: 12345679,  'NUMERO DE SERIAL': 987654321001},
      {SKU: 11112222, SUCURSAL: 'BIT MERCADO CDA LA CASTELLANA, C.A.',   IMEI: 12345679,  'NUMERO DE SERIAL': 987654321001},
      {SKU: 11112223, SUCURSAL: 'BIT MERCADO CDA LA CASTELLANA, C.A.',   IMEI: 12345680,  'NUMERO DE SERIAL': 987654321001},
      {SKU: 11112224, SUCURSAL: 'BIT MERCADO CDA LA CASTELLANA, C.A.',   IMEI: 12345681,  'NUMERO DE SERIAL': 987654321004},
    ];
    ...
    saveAs(blob, 'formato_inventario_importacion.xlsx');
  };
  ...
  const handleImport = async (file) => {
    ...
    try {
      ...
      setSelectedFile(null);
      setIsUploading(false);
      const el = document.getElementById('excel-file-upload');
      if (el) el.value = null;
    }
  };
  ...
  return (
    <>
      <button
        onClick={toggleModal}
        className="bg-oscuro text-white py-2 px-4 rounded-lg hover:bg-hover cursor-pointer mr-2"
      >
        Importar
      </button>
        ...
        <div className="space-y-4">
          <div className="flex justify-start space-x-4">
            <button
              ...
              className="flex items-center space-x-2 bg-oscuro text-white py-2 px-4 rounded-lg hover:bg-hover"
            >
              <FaDownload />
              <span>Formato</span>
            </button>

            <label className="flex items-center space-x-2 bg-oscuro text-white py-2 px-4 rounded-lg hover:bg-hover cursor-pointer">
              {isUploading ? (
                <>
          ...
          {importResults && typeof importResults === 'object' && !importResults.error && (
            <div className="p-4 rounded-l text-hover">
