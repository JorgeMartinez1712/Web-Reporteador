...
const DetallePagoPage = () => {
  ...
  const PaymentDetailsUI = ({ payment }) => {
    ...
    const getStatusColor = (status) => {
      ...
      if (s.includes('COMPLET') || s.includes('COMPLETADO') || s.includes('COMPLETED')) return 'bg-oscuro text-white';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center gap-3 mb-3">
                <i className="bi bi-person-fill text-oscuro text-2xl" />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center gap-3 mb-3">
                <i className="bi bi-credit-card text-oscuro text-2xl" />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center gap-3 mb-3">
                <i className="bi bi-arrow-right text-oscuro text-2xl" />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center gap-3 mb-3">
                <i className="bi bi-info-circle text-oscuro text-2xl" />
            ...
            {payment.notes && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center gap-3 mb-3">
                  <i className="bi bi-file-text text-oscuro text-2xl" />
          ...
          <div className="space-y-6">
            {(payment.retail_unit?.retail?.comercial_name || payment.retail_unit_name || payment.processed_by?.user?.name || payment.processed_by_name) && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center gap-3 mb-3">
                  <i className="bi bi-building text-oscuro text-2xl" />
                  ...
                  {payment.processed_by?.user?.name || payment.processed_by_name ? (
                    <div className='text-left'>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <i className="bi bi-person-check text-oscuro text-lg" />
            ...
            <div className="bg-gradient-to-br from-bg to-teal-50 border border-claro rounded-xl p-6">
              <h4 className="text-oscuro font-semibold text-left">Resumen</h4>
              <div className="space-y-3 mt-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-hover font-semibold">{payment.amount_converted ? `${currencyFormat(payment.amount_converted)} VES` : currencyFormat(payment.amount, currencySymbol)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Cuotas Aplicadas:</span>
                  <span className="text-hover font-semibold">{(payment.installments || []).length}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="max-w-full mx-auto  p-8">
      <div className="w-full flex justify-between items-center mb-6">
        <h2 className="text-xl font-extrabold text-hover tracking-tight">Detalle de pago</h2>
        <div className="flex items-center gap-3">
          {payment?.sale?.id && (
            <Link to={`/ventas/${payment.sale.id}`} className="bg-oscuro text-white py-2 px-4 rounded-lg hover:bg-hover cursor-pointer">
              Ver venta
            </Link>
          )}
        </div>
